...
export function triggerHeroSkill() {
  ...
  if (selectedHeroKey === 'KNIGHT') {
    ...
    // IGNIS: Dash contínuo e veloz com imunidade e queima térmica
    player.iFrames = 26;
    const moveAng = Math.atan2(inputY || 0.0001, inputX || player.facing);
    ...
    // KRAGDOR: Rugido Ancestral com Stun em área, anulação de projéteis e estado de Frenesi
    triggerShake(18);
    playSfx('boss');
    triggerHaptic('heavy');
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (dist < 260) {
        ...
        e.x += nx * 55;
        e.y += ny * 55;
        e.stunTimer = 110; // 1.8 segundos de atordoamento total
        e.slowTimer = 240;
        e.slowFactor = 0.65;
        e.hitFlash = 6;
    ...
    // Expurga projéteis inimigos no raio de 260px
    ...
    // 5 segundos de Frenesi com o dobro de rotação
...
// Translação Orbital Pura do Machado (Kragdor) com Corte de Projéteis Inimigos
export function updateSpinningAxes(dt) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // 1. Destruição física de projéteis inimigos na lâmina do machado
...
// Disparo Inteligente (Ataque condicionado à visão de alvos)
export function fireWeapons() {
  ...
  player.weapons.forEach(w => {
    ...
    if (w.type === 'SWORD') {
      ...
      // VALÉRIA: Predição de movimento e interceptação antecipada do alvo mais próximo
      playSfx('acid');
      ...
      // SIR ROLAND: Alinhamento frontal no alvo e impacto sísmico telegrafado
      playSfx('hit');
      triggerShake(10);
...
export function resetPlayer(heroKey) {
  ...
  player.xp = 0;
  player.nextXp = 5;
  player.damage = Math.round(c.stats.damage * meta.damageMult);
